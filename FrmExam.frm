   ...
   Begin Threed.SSPanel SSPanel1 
      ...
      Begin Threed.SSPanel PanSSA 
         ...
         Begin VB.ComboBox cmbrefS 
            Height          =   315
            ItemData        =   "FrmExam.frx":0000
            Left            =   1080
            List            =   "FrmExam.frx":0002
      ...
      Begin Threed.SSPanel SSPanel4 
         ...
         _ExtentY        =   1296
         _StockProps     =   15
         Caption         =   "Examination Results Administration"
         BackColor       =   14737632
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "01"
            Size            =   20.25
      ...
      Begin MSDBGrid.DBGrid grdGeneral 
         Bindings        =   "FrmExam.frx":0004
         Height          =   6375
         Left            =   4560
         OleObjectBlob   =   "FrmExam.frx":001D
         TabIndex        =   8
...
Dim FVal As Boolean, i As Integer
Dim AddFlag As Boolean, Emask As Integer
...
Dim rstMCRoom As Recordset, rstExams As Recordset
Dim rstInpTab As Recordset, rstSelRef As Recordset
Dim rstAutoB As Recordset, rstMarkS As Recordset
Dim rstDpro As Recordset

Dim MsubJCode, mSubjDesc, tempTaB As String
Dim MStaffNo, mStudno As String
Dim Emark As Double, StudCtr As Integer
Dim mCRCode, mCRDesc, mCRGrp As String
Dim mvMarks As Integer

Dim strpos As Integer, EFlag As Boolean
Dim FirstTime  As Boolean, Remarks As String
Dim GenClass As New DProLog, MarkPtr As Integer
...
Private Sub cmbRefL_Click()
   ...
   rstExams.FindFirst mvSql
   If rstExams.NOMATCH Then
      DoEvents
   Else
      evalSubjL = rstExams!subject & ", " & GTBuff
   End If
   'find subject reference end
...
Private Sub cmbrefS_Click()
   ...
   If mvAcad = True Then
      mvSql = "Select * from tabexam Where evalref = '" & GTBuff & "' and staffno = '" & mvUserid & "'"
   Else
      mvSql = "Select * from tabexam Where evalref = '" & GTBuff & "'"
   End If
...
On Error GoTo PError
   ...
   mvSql = "Select * from tabexam where evalref = '" & tempBuff & "'"
   Set rstSelClass = db1.OpenRecordset(mvSql, dbOpenDynaset)
...
On Error GoTo PError
   ...
   Else
      ...
      strpos = InStr(1, cmbrefS, ",", 1)
      GTBuff = Trim(Left(cmbrefS, strpos - 1))
      mvSql = "Select evdate,evalref,Studno,class,subject,score,staffno,remarks from  tabexam where evalref = '" & GTBuff & "'"
      ...
      Else
         Set datGeneral.Recordset = rst
         grdGeneral.Caption = Me.Caption & " - Examination Result Input"
...
On Error GoTo PError
   ...
   GTBuff = Trim(Left(cmbRefL, strpos - 1))
   Dim i As Integer
   If rstExams.RecordCount < 1 Then
   ...
   i = VoidCheck()
   If i = vbYes Then
       db1.Execute "DELETE * FROM tabexam where evalref = '" & GTBuff & "'"
       ...
       ' clear the grid
       If Not (rstExams.BOF And rstExams.EOF) Then
          mvSql = "Select Studno,subject,score from tabexam where evalref = '" & GTBuff & "'"
          Set rst = db1.OpenRecordset(mvSql, dbOpenDynaset)
          rst.Requery
          Set datGeneral.Recordset = rst
        Else
          Set datGeneral.Recordset = rstExams
        End If
        grdGeneral.Caption = Me.Caption
        Set datGeneral.Recordset = rst
        grdGeneral.Caption = Me.Caption & " - Examination Result Input"
...
On Error GoTo PError
     ...
     GenClass.fleLogin mvUserid, "Accessed Exams Administration", Date, Time
     ...
     Set rstExams = db1.OpenRecordset("tabexam", dbOpenDynaset)
     ...
     Set rstDpro = db1.OpenRecordset("Dproreg", dbOpenDynaset)
...
Dim MarkPtr As Integer
    ...
    If Not (rstInpTab.BOF And rstInpTab.EOF) Then
       ...
       Do While Not rstInpTab.EOF
          ...
          rstExams.AddNew
          rstExams("Evtype") = "EXAM"
          rstExams("Evalref") = UCase(NExRef)
          rstExams("studno") = MAStudno(MarkPtr)
          rstExams("class") = UCase(mCRCode)
          rstExams("subject") = UCase(MsubJCode)
          rstExams("score") = Emark
          rstExams("remarks") = Remarks
          rstExams("pyear") = Year(Date)
          rstExams("subperc") = 0
          rstExams("scoreval") = 0
          ...
          If rstSubj.NOMATCH Then
             ...
             rstExams("extint") = "XXXX"
          Else
             rstExams("extint") = rstSubj!extint
          End If
          If Len(Trim(teaCher)) = 0 Then
             rstExams("staffno") = UCase(mvUserid)
          Else
             rstExams("staffno") = teaCher
          End If
          rstExams("evdate") = evDate
          rstExams("entdate") = Date
          rstExams("schterm") = UCase(GCurrTerm)
          rstExams.Update
          '__________add  review to student notes
...
On Error GoTo PError
    If mvAcad = True Then
       mvSql = "Select * from tabexam Where staffno = '" & mvUserid & "'"
    Else
       mvSql = "Select * from tabexam"
    End If
...
On Error GoTo PError
     ...
     mvSql = "Select evdate,evalref,Studno,class,subject,score,staffno,remarks from  tabexam where studno = '" & mStudno & "' order by subject"
     Set rst = db1.OpenRecordset(mvSql, dbOpenDynaset)
     rst.Requery
     If (rst.BOF And rst.EOF) Then
         MsgBox "No Exam Results for Selected Student", vbExclamation, "Examination"
         Set datGeneral.Recordset = rst
         grdGeneral.Caption = Me.Caption
     Else
         Set datGeneral.Recordset = rst
         grdGeneral.Caption = "EXAMINATION RESULTS ON ALL SUBJECTS FOR " & tmpStr1
         Set Col0 = grdGeneral.Columns(0)
         Set Col1 = grdGeneral.Columns(1)
         Set Col2 = grdGeneral.Columns(2)
         Set Col3 = grdGeneral.Columns(3)
