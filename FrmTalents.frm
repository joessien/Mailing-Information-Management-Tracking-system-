...
Private Sub cmdFind_Click()
  Dim sql As String, digitflag As Integer, length As Integer
   ...
   AN = InputBox$("Enter the Talent Code to find:")
   length = Len(AN)
   AN = UCase(AN)
   If length = 0 Then
...
Private Sub Form_Load()
    ...
    GenClass.fleLogin mvUserid, "Accessed Talents Definition", Date, Time
    Call FormCentreMDI(Me)
    Set wrktemp = DBEngine.Workspaces(0)
    Set db1 = wrktemp.OpenDatabase(DBpath, True)
    Set rstDefTab = db1.OpenRecordset("DefTalents", dbOpenDynaset)
