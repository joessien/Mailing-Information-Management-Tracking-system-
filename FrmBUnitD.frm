...
Private Sub CmdSave_Click()
  Dim i As Integer
    ...
    If AddFlag = True Then
        rstDefTab.AddNew
        rstDefTab("rCode") = "U" & UCase(RCode)
    End If
    If Len(Trim(dEmail)) >= 1 Then
       EPos1 = InStr(1, dEmail, "@", 1)
       EPos2 = InStr(1, dEmail, ".", 1)
       If EPos1 = 0 Or EPos2 = 0 Or Len(dEmail) < 7 Then
          MsgBox "The Email given is invalid, Retry.", vbInformation, "Mail Records"
          dEmail.SetFocus
          Exit Sub
       End If
    End If
    rstDefTab("demail") = dEmail
...
Private Sub Form_Load()
    ...
    GenClass.fleLogin mvUserid, "Accessed Business Unit Definition", Date, Time
    Call FormCentreMDI(Me)
    Set wrktemp = DBEngine.Workspaces(0)
    Set db1 = wrktemp.OpenDatabase(DBpath, True)
    Set rstDefTab = db1.OpenRecordset("DefBUnitD", dbOpenDynaset)
...
Public Sub showdata()
     If rstDefTab.RecordCount >= 1 Then
        If IsNull(rstDefTab("RCode")) Then
           RCode = " "
        Else
           RCode = Right(rstDefTab("rCode"), 3)
        End If
        If IsNull(rstDefTab("rdesc")) Then
           RDesc = " "
        Else
           RDesc = rstDefTab("RDesc")
        End If
        If IsNull(rstDefTab("DEmail")) Then
           dEmail = " "
        Else
           dEmail = rstDefTab("demail")
        End If
    End If
End Sub
...
Private Sub DEmail_lostfocus()
    dEmail.SelStart = 0
    dEmail.SelLength = Len(BFname)
End Sub
