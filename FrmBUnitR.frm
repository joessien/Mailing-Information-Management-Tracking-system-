...
Private Sub cmdFind_Click()
  Dim sql As String, digitflag As Integer, length As Integer
   ...
   AN = InputBox$("Enter the Local Government Code to find:")
   length = Len(AN)
   AN = UCase(AN)
   If length = 0 Then
...
Private Sub CmdSave_Click()
  Dim i As Integer
    ...
    If AddFlag = True Then
        rstDefTab.AddNew
        rstDefTab("rCode") = "R" & UCase(RCode)
    End If
    rstDefTab("rDesc") = RDesc
    strpos = InStr(1, cmbGrp, ",", 1)
    rstDefTab("bunit") = Trim(Left(cmbGrp, strpos - 1))
    rstDefTab.Update
    AddFlag = False
    cmdadd.Caption = "&Add"
    CmdEnab
...
Private Sub dcb_Click(Area As Integer)
dcb.RowMember = "abc"
End Sub

Private Sub Form_Load()
    Dim flag As Integer, mtable As String
    GenClass.fleLogin mvUserid, "Accessed Business Unit Recipients Definition", Date, Time
    Call FormCentreMDI(Me)
    Set wrktemp = DBEngine.Workspaces(0)
    Set db1 = wrktemp.OpenDatabase(DBpath, True)
    Set rstDefTab = db1.OpenRecordset("DefBUnitR", dbOpenDynaset)
    Set rst = db1.OpenRecordset("defbunitd", dbOpenDynaset)
