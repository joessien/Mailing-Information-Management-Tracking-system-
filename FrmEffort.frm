   ...
   Begin Threed.SSPanel SSPanel1 
      ...
      Begin Threed.SSPanel PanSSA 
         ...
         Begin VB.ComboBox cmbrefS 
            Height          =   315
            ItemData        =   "FrmEffort.frx":0000
            Left            =   1080
            List            =   "FrmEffort.frx":0002
      ...
      Begin Threed.SSPanel SSPanel4 
         ...
         _ExtentY        =   1296
         _StockProps     =   15
         Caption         =   "Class Efforts Administration"
         BackColor       =   14737632
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "01"
            Size            =   20.25
      ...
      Begin MSDBGrid.DBGrid grdGeneral 
         Bindings        =   "FrmEffort.frx":0004
         Height          =   6375
         Left            =   4560
         OleObjectBlob   =   "FrmEffort.frx":001D
         TabIndex        =   21
      ...
      Begin VB.Label lblAms 
         ...
         Left            =   9480
         TabIndex        =   52
         ToolTipText     =   $"FrmEffort.frx":09F3
         Top             =   2400
         Width           =   975
      End
...
Dim rstAutoB As Recordset, rstMarkS, rstEvalsEffort As Recordset
...
Private Sub cmbRefL_Click()
   ...
   If mvAcad = True Then
      mvSql = "Select * from qryEvalsEffort Where evalref = '" & tmpStr1 & "' and staffno = '" & mvUserid & "'"
   Else
      mvSql = "Select * from qryEvalsEffort Where evalref = '" & tmpStr1 & "'"
   End If
   'Set rstSelClass = db1.OpenRecordset(mvSql, dbOpenDynaset)
   Set rstEvalsEffort = db1.OpenRecordset(mvSql, dbOpenDynaset)
   mvSql = "evalref = '" & Trim(Left(cmbRefL, strpos - 1)) & "'"
   'find subject reference end
   rstEvalsEffort.FindFirst mvSql
   If rstEvalsEffort.NOMATCH Then
      DoEvents
   Else
      evalSubjL = rstEvalsEffort!subject & ", " & GTBuff
   End If
   'find subject reference end
...
Private Sub cmbrefS_Click()
   ...
   If mvAcad = True Then
      mvSql = "Select * from qryEvalsEffort Where evalref = '" & GTBuff & "' and staffno = '" & mvUserid & "'"
   Else
      mvSql = "Select * from qryEvalsEffort Where evalref = '" & GTBuff & "'"
   End If
   Set rstSelClass = db1.OpenRecordset(mvSql, dbOpenDynaset)
   rstSelClass.Requery
...
On Error GoTo PError
   ...
   mvSql = "Select * from qryEvalsEffort where evalref = '" & tempBuff & "'"
   Set rstSelClass = db1.OpenRecordset(mvSql, dbOpenDynaset)
...
On Error GoTo PError
   ...
   Else
      ...
      strpos = InStr(1, cmbrefS, ",", 1)
      GTBuff = Trim(Left(cmbrefS, strpos - 1))
      mvSql = "Select evdate,evalref,Studno,class,subject,score,staffno,remarks from  qryEvalsEffort where evalref = '" & GTBuff & "'"
      ...
      Else
         Set datGeneral.Recordset = rst
         grdGeneral.Caption = Me.Caption & " - Class Attendance Input"
...
On Error GoTo PError
   ...
   If i = vbYes Then
       ...
       If mvAcad = True Then
          mvSql = "Select * from qryEvalsEffort Where Staffno = '"
          mvSql = mvSql & mvUserid & "'"
       Else
          mvSql = "Select * from qryEvalsEffort "
       End If
       Set rstEvalsEffort = db1.OpenRecordset(mvSql, dbOpenDynaset)
      'end ____________________
       ...
       If Not (rstEvals.BOF And rstEvals.EOF) Then
          mvSql = "Select Studno,subject,score from qryEvalsEffort where evalref = '" & GTBuff & "'"
          Set rst = db1.OpenRecordset(mvSql, dbOpenDynaset)
          rst.Requery
          Set datGeneral.Recordset = rst
        Else
...
On Error GoTo PError
    If mvAcad = True Then
       mvSql = "Select * from qryEvalsEffort Where staffno = '" & mvUserid & "'"
    Else
       mvSql = "Select * from qryEvalsEffort "
    End If
...
On Error GoTo PError
     ...
     mvSql = "Select evdate,evalref,Studno,class,subject,score,staffno,remarks from  tabevals where studno = '" & mStudno & "' and Evtype = 'EFFORT'" & " order by subject"
     Set rst = db1.OpenRecordset(mvSql, dbOpenDynaset)
     rst.Requery
     If (rst.BOF And rst.EOF) Then
     ...
     Else
         Set datGeneral.Recordset = rst
         grdGeneral.Caption = "ATTENDANCE RESULTS ON ALL SUBJECTS FOR " & tmpStr1
         Set Col0 = grdGeneral.Columns(0)
         Set Col1 = grdGeneral.Columns(1)
         Set Col2 = grdGeneral.Columns(2)
         Set Col3 = grdGeneral.Columns(3)
